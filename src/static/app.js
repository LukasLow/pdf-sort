...
async function loadNextFile() {
	...
	document.getElementById('pdf-viewer').src =
		"/view-pdf/" + encodeURIComponent(currentFile);
	...
	document.getElementById('ppi').innerText =
		data.ppi || "-";

	globalConfig = data.config_data || {};

	renderDropdowns(
		data.suggested_correspondent,
		data.suggested_info
	);
...
function renderDropdowns(selectedCorr, selectedInfo) {
	...
	if (selectedInfo) {
		document.getElementById('sel-info').value =
			selectedInfo;
	}
}
...
function onCorrespondentChange() {
	const corr =
		document.getElementById('sel-correspondent').value;

	const infoSelect =
		document.getElementById('sel-info');
...
function updatePreview() {
	...
	document.getElementById('filename-preview').innerText =
		txt;
}
